...
function isEnrollmentPaymentIntentSnapshot(
  value: unknown
): value is EnrollmentPaymentIntentSnapshot {
...
export function StudentIntakeModal({ open, onClose }: StudentIntakeModalProps) {
  ...
  const [paymentIntent, setPaymentIntent] = React.useState<EnrollmentPaymentIntentSnapshot | null>(
    null
  );
  ...
  const studentId = React.useMemo(
    () => syncedUser?.localUserId || 'student-001',
  ...
  React.useEffect(() => {
    setPaymentIntent((current) =>
      current && current.cohortId !== selectedCohortId ? null : current
    );
  }, [selectedCohortId]);
  ...
  const handleSubmitExam = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/students/${studentId}/intake/entrance-exam/submit`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${session.access_token}`,
  ...
  const requiredDocumentsUploaded = requiredDocuments.every(
    (document) => readinessUploads[document.id]
  );
  ...
  const surveyStepValid = currentSurveySection
    ? currentSurveySection.fields.every((field) => (entranceSurvey.answers[field.id] ?? '').trim())
    : false;
      ...
      <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[28px] bg-surface shadow-2xl">
        <div className="shrink-0 flex items-start justify-between gap-6 border-b border-border-subtle bg-primary px-6 py-5 text-white sm:px-8">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-white/80">{journey.header.description}</p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-on-surface-variant mt-1">
                  Your intake has been submitted and is pending admin approval. You will be allowed
                  to use the platform once approved.
                </p>
          ...
          <div className="shrink-0 border-b border-warning/20 bg-warning/5 px-6 py-4 sm:px-8">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-on-surface-variant mt-1">
                  This intake flow can only run with a synced student account. The current session
                  is not authorized for student intake requests.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="shrink-0 border-b border-info/20 bg-info/5 px-6 py-4 sm:px-8">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-on-surface-variant mt-1">
                    Complete the entrance exam and upload the required documents, then submit your
                    application for admin review. Cohort selection and payment unlock after you're
                    approved.
                  </p>
                </div>
              </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6 sm:px-8">
                ...
                {entranceExam.taken ? (
                  ...
                  <div className="space-y-5">
                    {examQuestions.map((question, index) => (
                      <div
                        key={question.id}
                        className="rounded-[20px] border border-border-subtle bg-surface p-5"
                      >
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            {question.options.map((option) => (
                              <button
                                key={option.value}
                                onClick={() =>
                                  answerEntranceExamQuestion(question.id, option.value)
                                }
                                className={cn(
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                              ...
                              >
                                {isUploading ? 'Uploading...' : uploaded ? 'Update' : 'Upload'}
                              </Button>
                            </div>
                          </div>
                        );
            ...
            {workflowStage === 'enrollment_wizard' && approvalStatus !== 'approved' ? (
              <section className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="mt-2 text-sm text-on-surface-variant max-w-md">
                  Your application must be approved by admin before you can proceed. Please wait for
            ...
            !needsEnrollmentConfirmation ? (
              <section className="space-y-6">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <Badge variant="info">
                      Step {wizardStep} of {enrollmentSteps.length}
                    </Badge>
                  ...
                  <div className="h-2 w-44 overflow-hidden rounded-full bg-surface-container">
                    <div
                      ...
                      style={{
                        width: `${(wizardStep / Math.max(enrollmentSteps.length, 1)) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                    ...
                    <div className="rounded-[22px] border border-border-subtle bg-surface-muted p-6">
                      ...
                      <div className="mt-4 grid gap-4 sm:grid-cols-2">
                        <Select
                          ...
                          placeholder={
                            currentWizardDefinition?.sections[0]?.fields[0]?.label ??
                            'Scrub top size'
                          }
                        ...
                        <Select
                          ...
                          placeholder={
                            currentWizardDefinition?.sections[0]?.fields[1]?.label ??
                            'Scrub bottom size'
                          }
                  ...
                  <div className="rounded-[22px] border border-border-subtle bg-surface-muted p-6">
                    ...
                    <div className="mt-5 space-y-3">
                      {currentWizardDefinition?.sections[0]?.fields.map((field) => {
                        ...
                        const checked = enrollmentWizard.agreements[key];

                        return (
                          <button
                            key={field.id}
                            onClick={() => toggleEnrollmentAgreement(key)}
                            className={cn(
                  ...
                  <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                    <div className="rounded-[22px] border border-border-subtle bg-surface-muted p-6">
                      ...
                      <div className="mt-5 rounded-[18px] bg-surface p-5">
                        <p className="text-sm text-on-surface-variant">
                          Registration, tuition, optional HHA add-on, and delivery preference are
                          staged in the enrollment packet for review.
                        </p>
                        <Input
            ...
            {workflowStage === 'orientation_survey' && approvalStatus !== 'approved' ? (
              <section className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="mt-2 text-sm text-on-surface-variant max-w-md">
                  Your application must be approved by admin before proceeding. Check back after
                  approval.
                </p>
              </section>
            ) : null}
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="h-2 w-44 overflow-hidden rounded-full bg-surface-container">
                    <div
                      ...
                      style={{
                        width: `${(entranceSurvey.step / Math.max(surveySections.length, 1)) * 100}%`,
                      }}
                    />
                  </div>
                </div>
