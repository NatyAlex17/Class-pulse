...
type WorkspaceView = 'modules' | 'module-detail' | 'section-detail';
...
export function LearningResourcesConfigWorkspace({ view, moduleId, sectionId }: WorkspaceProps) {
  const {
    config,
    setConfig,
    loading,
  ...
  if (loading) {
    return (
      <AdminShell
        ...
        subtitle="Loading module and section configuration."
      >
  ...
  if (view === 'section-detail' && (!selectedModule || !selectedSection)) {
    return (
      <AdminShell title="Section Not Found" subtitle="Return to the section list to choose another section.">
        <Link href="/admin/configurations/learning-resources">
          <Button variant="secondary">
            <IconArrowLeft className="size-4" />
            Back to Modules
  ...
  if (view === 'modules') {
    ...
    return (
      <AdminShell
        title="Learning Management Config"
        subtitle="Start with the module list. Click one module to manage its sections."
        topActions={
          <div className="flex items-center gap-2">
        ...
        <div className="space-y-6">
              ...
              <div className="mt-5">
                <Button
                  onClick={() => {
                    ...
                    setConfig((current) =>
                      current
                        ? { ...current, modules: [...current.modules, nextModule] }
                        : current,
                    );
          ...
          <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-on-surface-variant">
                Click any row to open that module's detail page and manage its sections.
              </p>
            </div>
            <DataTable
              columns={columns}
              ...
              getRowId={(row) => row.id}
              onRowClick={(row) => {
                window.location.href = getModuleHref(row.id);
              }}
  ...
  if (view === 'module-detail' && selectedModule) {
    ...
    const columns: DataTableColumn<SectionRow>[] = [
      ...
      { id: 'items', header: 'Items', accessorKey: 'items' },
      { id: 'description', header: 'Description', accessorKey: 'description', mobileHidden: true },
    ];

    return (
      <AdminShell
        title={selectedModule.title}
        subtitle="Module detail page. Edit the module here, then open a section to manage its learning items."
        topActions={
          <div className="flex items-center gap-2">
          ...
          <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-3 text-sm text-on-surface-variant">
                  Update the module first. Sections stay on their own page below this level.
                </p>
              </div>
              <Button
                variant="destructive"
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    updateModule(selectedModule.id, (module) => ({ ...module, title: event.target.value }))
                  }
                />
              </div>
              <div>
            ...
            <div className="mt-4">
              ...
              <Textarea
                ...
                onChange={(event) =>
                  updateModule(selectedModule.id, (module) => ({ ...module, summary: event.target.value }))
                }
              />
            </div>
          </div>
            ...
            <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-on-surface-variant">
                    This section will be added inside <strong>{selectedModule.title}</strong>.
                  </p>
                </div>
          ...
          <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-on-surface-variant">
                  Click one section to open its detail page and manage its learning items.
                </p>
              </div>
            ...
            <DataTable
              ...
              getRowId={(row) => row.id}
              onRowClick={(row) => {
                window.location.href = getSectionHref(selectedModule.id, row.id);
              }}
              mobileCardTitle={(row) => row.title}
  ...
  if (view === 'section-detail' && selectedModule && selectedSection) {
    ...
    const columns: DataTableColumn<ResourceRow>[] = [
      { id: 'title', header: 'Learning Item', accessorKey: 'title' },
      {
        id: 'type',
        header: 'Type',
        cell: (row) => <Badge variant={row.type === 'exam' ? 'warning' : 'info'}>{resourceTypeLabels[row.type]}</Badge>,
      },
      { id: 'duration', header: 'Duration', accessorKey: 'duration' },
    ];

    const editableItem = selectedSection.resources[0] ?? null;

    return (
      <AdminShell
        title={selectedSection.title}
        subtitle="Section detail page. Manage this section and all items inside it from here."
        topActions={
          <div className="flex items-center gap-2">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-3 text-sm text-on-surface-variant">
                  Update the section details here, then manage the section items below.
                </p>
              </div>
              <Button
                variant="destructive"
            ...
            <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
              <div className="mb-5 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-on-surface-variant">
                    This item will be added inside <strong>{selectedSection.title}</strong>.
                  </p>
                </div>
          ...
          <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-on-surface-variant">
                  This section owns all of the content items below.
                </p>
              </div>
          ...
          {editableItem ? (
            <div className="rounded-[20px] border border-border-subtle bg-surface p-6 shadow-soft">
              <div className="mb-5 flex items-start justify-between gap-3">
                <div>
                  <h2 className="font-display text-[22px] font-semibold text-on-surface">First Item Editor</h2>
                  <p className="mt-1 text-sm text-on-surface-variant">
                    Focused editor for the first section item. We can expand this into per-item pages next if you want.
                  </p>
                </div>
                <Button
                  variant="destructive"
                  ...
                  onClick={() =>
                    updateSection(selectedModule.id, selectedSection.id, (section) => ({
                      ...section,
                      resources: section.resources.filter((resource) => resource.id !== editableItem.id),
                    }))
                  }
                >
                  <IconTrash className="size-4" />
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      updateResource(selectedModule.id, selectedSection.id, editableItem.id, (resource) => ({
                        ...resource,
                        title: event.target.value,
                      }))
                    }
                ...
                <div>
                  ...
                  <Input value={editableItem.id} disabled />
                </div>
              </div>
